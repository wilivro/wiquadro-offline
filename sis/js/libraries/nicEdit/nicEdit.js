...
var nicUploadButton = nicEditorAdvancedButton.extend({nicURI: "http://api.imgur.com/2/upload.json", errorText: "Failed to upload image", addPane: function() {
        ...
        /* 
         * Alterado por David Figueiredo em 22/11/2013
         * Adaptação da lógica de upload de imagens para o uso da API FileReader
         * 
         */
            ...
            reader.onload = (function(theFile)
            {
                return function(e)
                {
                    ...
                    function(r)
                    {
                        ...
                        {
                            element.removePane();
                            var D = r.upload.links.original;
                            if (!element.im) {
                                element.ne.selectedInstance.restoreRng();
                                var C = "javascript:nicImTemp();";
                                element.ne.nicCommand("insertImage", D);
                                element.im = element.findElm("IMG", "src", D);
                            }
                            var A = parseInt(element.ne.selectedInstance.elm.getStyle("width"));
                            if (element.im) {
                                element.im.setAttributes({src: D, width: (A && B.image.width) ? Math.min(A, B.image.width) : ""});
                            }
                        }
                        else
                        {
        ...
        }.closure(this)//this.uploadFile.closure(this)
